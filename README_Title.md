...
论文翻译的校对和体例统一工作由吴咏炜、杨文波、张云潮合力进行，最后的发布由吴咏炜完成。
...
翻译过程中我们发现了一些原文中的小问题，并在译文中进行了修正或标注（绝大部分已经经过 Bjarne 老爷子确认）。然而，在翻译过程中引入翻译错误或其他技术问题，恐怕也在所难免。不过，跟 ACM 上发表论文不同，这个网页仍然是可以修正的。所以，如果你发现问题的话，请不吝提交 pull request，我们会尽快检查并进行修正。
